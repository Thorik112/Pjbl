...
if (isset($_SESSION['level']) && isset($_SESSION['nama'])) {
    if ($_SESSION['level'] == "admin") {
        // Tidak ada tindakan khusus yang dilakukan untuk admin dalam blok ini
...
function getNotifications($penjoki) {
    if (isset($_SESSION['notif2'][$penjoki])) {
        return $_SESSION['notif2'][$penjoki];
    }
    return array();
}

$penjoki = $_SESSION['nama'];

// Tampilkan notifikasi kepada pengguna
$notifications = getNotifications($penjoki);
?>

<?php if (!empty($notifications)) { ?>
    <script>
        var notification = <?php echo json_encode(array_shift($notifications)); ?>;
        alert(notification);
        // Atau Anda dapat menggunakan elemen HTML atau komponen modal untuk menampilkan notifikasi
        // dengan cara yang lebih terstruktur dan menarik.
    </script>
<?php } ?>
...
?>
	...
	<div class="container" style="margin-bottom: 5%; margin-top: 5%;">
        <h2>Kelola Pesanan Joki</h2>
				<?php
				$nama = $_SESSION['nama'];
                            include "koneksi.php";
                            $query = "SELECT * FROM transaksi_joki WHERE nama_penjoki='$nama' AND (hasil = 'menunggu' OR hasil = 'diproses') ORDER BY id_joki ASC";
                            $hasil = mysqli_query($koneksi, $query);
                            $no = 1;
                            $jum = mysqli_num_rows($hasil);
        ...
        <table id="example" class="table table-striped table-bordered" style="margin-bottom: 10%;">
            <thead>
                <tr>
                    <th scope="col">ID</th>
              ...
              while ($data = mysqli_fetch_array($hasil)) {
                ...
                <tr>
                    ...
                    <td><?php echo $data['nama_pemesan']; ?></td>
										...
										<td><?php echo $data['username_game']; ?></td>
										<td><?= $data['hasil'] === 'menunggu' ? '<h5><span class="badge badge-warning">Menunggu</span></h5>' 
										: ($data['hasil'] === 'ditolak' ? '<h5><span class="badge badge-danger">DiTolakspan></h5>' 
										:'<h5><span class="badge badge-success">DiProses</span></h5>') ?></td>
										<td>
											<?php
											if($data['hasil'] == 'menunggu'){
											?>
											<form action="proses_joki.php" method="post" id="tolak-form">
													<input type="hidden" name="id" value="<?= $data['id_joki'] ?>">
													<input type="hidden" name="nama_pemesan" value="<?= $data['nama_pemesan'] ?>">
													<input type="hidden" name="action" value="ditolak">
													<button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Tolak Joki Ini?')">
															<i>TOLAK</i>
													</button>
											</form>

											<form action="proses_joki.php" method="post">
													<input type="hidden" name="id" value="<?= $data['id_joki'] ?>">
													<input type="hidden" name="nama_pemesan" value="<?= $data['nama_pemesan'] ?>">
													<input type="hidden" name="action" value="diproses">
													<button type="submit" class="btn btn-success btn-sm" onclick="return confirm('Setujui Joki Ini?')">
															<i>TERIMA</i>
													</button>
													
											</form>
											<script>
													// Fungsi untuk mengirim formulir "TOLAK" secara otomatis setelah satu menit
													function autoSubmitTolakForm() {
															document.getElementById("tolak-form").submit();
													}

													// Panggil fungsi autoSubmitTolakForm setelah satu menit (60,000 milidetik)
													setTimeout(autoSubmitTolakForm, 60000);
											</script>

												<?php
												}
												else if($data['hasil'] == 'diproses'){
												?>
												<form action="proses_joki.php" method="post" enctype="multipart/form-data">
												<div class="form-group">
                                <label for="gambar">Bukti Joki:</label>
                                <input type="file" name="gambar" id="gambar" required>
														...
														<input type="hidden" name="id" value="<?= $data['id_joki'] ?>">
														<input type="hidden" name="nama_pemesan" value="<?= $data['nama_pemesan'] ?>">
														<button type="submit" name="action" value="selesai" class="btn btn-success btn-sm" onclick="return confirm('Joki Telah Selesai?')">
																<i>Selesai</i>
														</button>
                            </div>
												</form>
