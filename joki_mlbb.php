		...
		<div class="container">
			<div class="row">
				<div class="col-lg-4 mb-5 mb-lg-0">
					<h4 class="comment-title">Joki MLBB</h4>
				...
				<div class="col-lg-8">
					<div class="contact-form-warp">
						...
						<h6>Harga/Bintang</h6>
						<p>Warrior 1000 | Elite 1000 | Master 1500 | GrandMaster 2000 | Epic 2500 | Legend 3000 | Mythic Mythical Honor 4000 | Mythical Glory 5000 | Mythical Immortal 6000<p>
							<div class="row">
								<div class="col-md-12">
								...
								<div class="input_group">
											<div class="input_box">
												<h6>Rank</h6>
												<input type="radio" required id="b1" name="rank" class="radio" value="warrior">
												<label for="b1">Warrior</label>
												<input type="radio" id="b11" name="rank" class="radio" value="elite">
												<label for="b11">Elite</label>
												<input type="radio" id="b12" name="rank" class="radio" value="master">
												<label for="b12">Master</label>
												<input type="radio" id="b13" name="rank" class="radio" value="grandmaster">
												<label for="b13">GrandMaster</label>
												<input type="radio" id="b14" name="rank" class="radio" value="epic">
												<label for="b14">Epic</label>
												<input type="radio" id="b6" name="rank" class="radio" value="legend">
												<label for="b6">Legend</label>
												<input type="radio" id="b7" name="rank" class="radio" value="mythic">
												<label for="b7">Mythic</label>
												<input type="radio" id="b8" name="rank" class="radio" value="mythical_honor">
												<label for="b8">Mythical Honor</label>
												<input type="radio" id="b9" name="rank" class="radio" value="mythical_glory">
												<label for="b9">Mythical Glory</label>
												<input type="radio" id="b10" name="rank" class="radio" value="mythical_immortal">
												<label for="b10">Mythical Immortal</label>
											</div>
								</div>
									<div class="col-lg-12">
										<input type="number" placeholder="Berapa Bintang" id="bintang" required name="bintang">
								</div>
								<div class="col-lg-12">
			...
			<div class="input_box">
				...
				<script>
										...
										var game2 = document.getElementById("b11");
										var game3 = document.getElementById("b12");
										var game4 = document.getElementById("b13");
										var game5 = document.getElementById("b14");
										var game6 = document.getElementById("b6");
										var game7 = document.getElementById("b7");
										var game8 = document.getElementById("b8");
										var game9 = document.getElementById("b9");
										var game10 = document.getElementById("b10");
										...
										game7.addEventListener("change", updateTotal);
										game8.addEventListener("change", updateTotal);
										game9.addEventListener("change", updateTotal);
										game10.addEventListener("change", updateTotal);
										...
										bintang.addEventListener("input", function() {
											...
											priceElem.textContent = "Rp" + updateTotal();
										});

										function updateTotal() {
											var totalBayar = 0;
											if (game1.checked) {
												totalBayar += 1000 * parseFloat(bintang.value);
											}
											if (game2.checked) {
												totalBayar += 1000 * parseFloat(bintang.value);
											}
											if (game3.checked) {
												totalBayar += 1500 * parseFloat(bintang.value);
											}
											if (game4.checked) {
												totalBayar += 2000 * parseFloat(bintang.value);
											}
											if (game5.checked) {
												totalBayar += 2500 * parseFloat(bintang.value);
											}
											if (game6.checked) {
												totalBayar += 3000 * parseFloat(bintang.value);
											}
											if (game7.checked) {
												totalBayar += 4000 * parseFloat(bintang.value);
											}
											if (game8.checked) {
												totalBayar += 4000 * parseFloat(bintang.value);
											}
											if (game9.checked) {
												totalBayar += 5000 * parseFloat(bintang.value);
											}
											if (game10.checked) {
												totalBayar += 6000 * parseFloat(bintang.value);
											}

											harga.value = totalBayar;
											price.value = totalBayar;
											return totalBayar;
										}
										...
										var priceText = priceElem.textContent; // Mengambil teks dari totalBayarElem
										var priceBayar = parseFloat(priceText.replace("Rp ", "")); // Mengambil nilai numerik dari teks
...
function transaksiBaru($penjoki, $username) {
    // Logika untuk menyetujui transaksi
    // Anda juga bisa menambahkan notifikasi ke pengguna
    $adminMessage = "Ada Pesanan joki baru yang dipesan oleh $username.";
...
if ($kirim) {
    ...
    if ($koinPengguna >= $totalBayar) {
        ...
        if ($koneksi->query($query) === TRUE) {
					...
					$mail->Password = "ruunoqvitymlgjfz";           //password email smtp
					...
					$mail->Subject= "Ada Pesanan Joki Mlbb"; //subject
						$mail->Body    = "Tanggal Pemesanan $tanggal Harga $totalBayar penjoki $penjoki"; //isi email
            ...
            echo "<script>window.alert('Pesanan joki gagal terkirim');
            window.location='pesan.php';</script>";
        }
    } else {
...
<!-- Link back to Colorlib can't be removed. Template is licensed under CC BY 3.0. -->
    ...
    // Fungsi untuk memperbarui total harga dan diskon
    function updateTotal() {
        ...
        if(coupon == ""){
            harga.value = price; // Update total jika kupon tidak dimasukkan
            $('#result').html('');
        } else {
            $.post('get_discount.php', {coupon: coupon, price: price}, function(data){
                if(data == "error"){
                    alert("Invalid Coupon Code!");
                    harga.value = price; // Update total jika kupon tidak valid
                    $('#result').html('');
                } else {
                    var json = JSON.parse(data);
                    $('#result').html("<h4 class='pull-right text-danger'>"+json.discount+"% Off</h4>");
                    harga.value = json.price; // Update total dengan diskon dari kupon
                }
            });
        }
    }

    // Event handler untuk perubahan pada input bintang
    $('#bintang').on('input', function() {
        updateTotal();
    });

    // Event handler untuk perubahan pada game
    $('input[name^="rank"]').on('change', function() {
        updateTotal();
    });

    // Event handler untuk kupon
