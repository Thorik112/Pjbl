...
?>
	...
	<div class="container" style="margin-bottom: 5%; margin-top: 5%;">
        <h2>Kelola Pesanan Mabar</h2>
				<?php
				$nama = $_SESSION['nama'];
                            include "koneksi.php";
                            $query = "SELECT * FROM transaksi_mabar WHERE nama_pemain='$nama' AND hasil = 'menunggu' OR hasil = 'diproses' ORDER BY id_mabar ASC";
                            $hasil = mysqli_query($koneksi, $query);
                            $no = 1;
                            $jum = mysqli_num_rows($hasil);
                ...
                <tr>
                    ...
                    <td><?php echo $data['nama_pemesan']; ?></td>
										...
										<td>
											...
											?>
											<form action="proses_mabar.php" method="post">
													<input type="hidden" name="id" value="<?= $data['id_mabar'] ?>">
													<input type="hidden" name="nama_pemesan" value="<?= $data['nama_pemesan'] ?>">
													<button type="submit" name="action" value="ditolak" class="btn btn-danger btn-sm" onclick="return confirm('Tolak Joki Ini?')">
															<i>TOLAK</i>
													</button>
											</form>
											...
											<form action="proses_mabar.php" method="post">
													<input type="hidden" name="id" value="<?= $data['id_mabar'] ?>">
													<input type="hidden" name="nama_pemesan" value="<?= $data['nama_pemesan'] ?>">
													<button type="submit" name="action" value="diproses" class="btn btn-success btn-sm" onclick="return confirm('Setujui Joki Ini?')">
															<i>TERIMA</i>
													</button>
											</form>
												<?php
												...
												?>
												<form action="proses_mabar.php?id=<?= $data['id_mabar'] ?>&&h=selesai" method="post" enctype="multipart/form-data">
												<div class="form-group">
                                <label for="gambar">Bukti Mabar:</label>
