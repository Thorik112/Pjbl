...
?>
	...
	<?php 
         include "koneksi.php";
				 $nama = $_SESSION['nama'];
         $query = "SELECT * FROM transaksi_joki WHERE nama_pemesan = '$nama'";
         $hasil = mysqli_query($koneksi, $query);
				 $no = 1;
				 $jum = mysqli_num_rows($hasil);
				 if ($jum > 0) {
    ...
    : ($data['hasil'] === 'ditolak' ? '<h5><span class="badge badge-danger">DiTolak</span></h5>' 
        : ($data['hasil'] === 'selesai' ? '<h5><span class="badge badge-success">Selesai</span></h5>' 
            : '<h5><span class="badge badge-success">DiProses</span></h5>')) ?>

            </div>
            <div class="transaction-details text-white">
                <p>Nama Game: <?php echo $data['nama_game']; ?></p>
                <p>Nama Penjoki: <?php echo $data['nama_penjoki']; ?></p>
				...
				?>
				<h3 style="text-align: center; margin-top: 40px; margin-bottom: 40px;">Anda Belum Melakukan Pesanan Joki Apapun</h3>
				<div class="cata new mt-2" style="margin-left: 45%; margin-bottom: 40px;"><a href="mabar.php">Pesan Sekarang</a></div>
				<?php
				}
				?>
